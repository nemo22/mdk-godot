...
## over the view (holes for the scope and the three round cameras at the top, filled black while
## those cameras aren't drawn), the `CROSS` crosshair on the scope's centre, the zoom in percent
## with its `SNIP_RNG` gauge, and the ammo types (`SNIP_WEP`, `SNIP_Wn`, the selected one's
...
## ends. The holes outside the scope are filled black (the round cameras aren't drawn yet).
static func _mask_image(sprites: MDKBni, palette: MDKPalette) -> Image:
	...
	while pixel < 600 * 360:
		...
		if word == 0xFF00:
			...
			for i in word & 0xFFF:
				var point := Vector2(pixel % 600, pixel / 600)
				if not SCOPE.has_point(point) and pixel < 600 * 360:
					image.set_pixel(pixel % 600, pixel / 600, Color.BLACK)
				pixel += 1
			continue
		else:
			count = word * 4
...
func draw(canvas: HUD, kurt: Kurt) -> void:
